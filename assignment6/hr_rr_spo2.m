...
[ir_peak_high,ir_peak_locs] = findpeaks(ir,1,'MinPeakDistance',5);
[red_peak_high,red_peak_locs] = findpeaks(red,1,'MinPeakDistance',5);
...
[ir_valley_high,ir_valley_locs] = findpeaks(ir_n,1,'MinPeakDistance',5);
...
rr_ir = bandp(ir,1/6,1/3,0.01,2/3,0.1,30,10);% bandpass 1/6HZ-1/3HZ
rr_ir = smooth(rr_ir);
[rr_ir_high,rr_ir_locs] = findpeaks(rr_ir,1,'MinPeakDistance',30);
...
for i=1:1:rr_ir_locs_count-1
    resp_rate(i) = round(60 *10 /(rr_ir_locs(i+1) - rr_ir_locs(i)));
end
...
for i=1:1:ir_valley_locs_count-1
    ...
    heartrate(i) = round(60 * 10 / (ir_peak_locs(i+1) - ir_peak_locs(i)));
end
%figure;
subplot(311);plot(heartrate);
subplot(312);plot(resp_rate);
subplot(313);plot(spo2_cal);
%figure;
