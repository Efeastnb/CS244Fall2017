...
spo2_cal = zeros(ir_valley_locs_count-80, 1);
heartrate = zeros(ir_valley_locs_count-80, 1);
...
rr_ir = bandp(ir,0.2,0.4,0.05,2/3,0.1,30,50);% bandpass 1/6HZ-1/3HZ
rr_ir = smooth(rr_ir);
[rr_ir_high,rr_ir_locs] = findpeaks(rr_ir,1,'MinPeakDistance',150);
%findpeaks(rr_ir,1,'MinPeakDistance',30);
rr_ir_locs_count = length(rr_ir_locs);
resp_rate = zeros(rr_ir_locs_count-11, 1);
for i=1:1:rr_ir_locs_count-11
    resp_rate(i) = round(60 * 10 * 50 /(rr_ir_locs(i+11) - rr_ir_locs(i)));
end

%use an_ratio betwen AC component of Ir & Red and DC component of Ir & Red for SPO2 
%calculate heartrate
for i=1:1:ir_valley_locs_count-80
    ...
    if spo2_cal(i) < 91 || spo2_cal(i) > 99
        spo2_cal(i) = 95;
    end
    heartrate(i) = round(60 * 60 * 50 / (ir_peak_locs(i+61) - ir_peak_locs(i)));
end
%figure;
subplot(311);plot(heartrate);title('HeartRate');
subplot(312);plot(resp_rate);title('Respration Rate');
subplot(313);plot(spo2_cal);title('Spo2');
%figure;
...
%findpeaks(ir,1,'MinPeakDistance',5);
%findpeaks(ir_n,1,'MinPeakDistance',5);title('Find Valleys');
